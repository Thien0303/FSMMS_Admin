...
import { getAllPost, updateAllPost} from "../../../redux/apiThunk/ExpertThunk/postThunk";
...
export default function SinglePost() {
  ...
  const handleUpdateSubmit = async (values, { setSubmitting, resetForm }) => {
    try {
      ...
      await dispatch(updateAllPost({ id: selectedPostData.postId, data: formData }));
      handleReload();
    ...
    <>
      {postData.map((post) => (
        <Card key={post.postId} sx={{ maxWidth: "50vw", m: "auto", mb: 5}}>
          ...
          <CardMedia component="img" height={100} image={post.postImage} alt="alt name" sx={{width: "100%", height: "100%", objectFit: "contain"}} />
          </NavLink>
          <CardContent>
  ...
  {selectedPostId === post.postId && (
    ...
    >
      ...
      <MenuItem onClick={() => handleDeletePost(post.postId)}>
        <DeleteForeverOutlinedIcon fontSize="small" color="error" /> Delete
      </MenuItem>
    </Menu>
  )}
</span>
